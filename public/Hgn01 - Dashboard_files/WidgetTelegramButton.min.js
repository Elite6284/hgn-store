    ...
    if (typeof WidgetTelegramButton != "undefined") var _WidgetTelegramButton = WidgetTelegramButton; var WidgetTelegramButton = window.WidgetTelegramButton = function (a, c) { if (window == this || !this.init) return new WidgetTelegramButton(a, c); return this.init(a, c); }; if (typeof $ != "undefined") var _$ = $; window.$ = WidgetTelegramButton; var quickExpr = /^[^<]*(<(.|\s)+>)[^>]*$|^#(\w+)$/; WidgetTelegramButton.fn = WidgetTelegramButton.prototype = {
        init: function (a, c) {
            ...
            var clone = this.length > 1, a; return this.each(function () {
                if (!a) { a = WidgetTelegramButton.clean(args, this.ownerDocument); if (dir < 0) a.reverse(); } var obj = this; if (table && WidgetTelegramButton.nodeName(this, "table") && WidgetTelegramButton.nodeName(a[0], "tr")) obj = this.getElementsByTagName("tbody")[0] || this.appendChild(document.createElement("tbody")); WidgetTelegramButton.each(a, function () {
                    if (WidgetTelegramButton.nodeName(this, "script")) {
                        if (this.src) WidgetTelegramButton.ajax({ url: this.src, async: false, dataType: "script" }); else
                            WidgetTelegramButton.globalEval(this.text || this.textContent || this.innerHTML || "");
                    } else
                        fn.apply(obj, [clone ? this.cloneNode(true) : this]);
                });
            });
        }
    }; WidgetTelegramButton.extend = WidgetTelegramButton.fn.extend = function () { var target = arguments[0] || {}, a = 1, al = arguments.length, deep = false; if (target.constructor == Boolean) { deep = target; target = arguments[1] || {}; } if (al == 1) { target = this; a = 0; } var prop; for (; a < al; a++)if ((prop = arguments[a]) != null) for (var i in prop) { if (target == prop[i]) continue; if (deep && typeof prop[i] == 'object' && target[i]) WidgetTelegramButton.extend(target[i], prop[i]); else if (prop[i] != undefined) target[i] = prop[i]; } return target; }; var expando = "WidgetTelegramButton" + (new Date()).getTime(), uuid = 0, win = {}; WidgetTelegramButton.extend({
        noConflict: function (deep) { window.$ = _$; if (deep) window.WidgetTelegramButton = _WidgetTelegramButton; return WidgetTelegramButton; }, isFunction: function (fn) { return !!fn && typeof fn != "string" && !fn.nodeName && fn.constructor != Array && /function/i.test(fn + ""); }, isXMLDoc: function (elem) { return elem.documentElement && !elem.body || elem.tagName && elem.ownerDocument && !elem.ownerDocument.body; }, globalEval: function (data) {
            ...
            var r = []; doc = doc || document; WidgetTelegramButton.each(a, function (i, arg) {
                if (!arg) return; if (arg.constructor == Number) arg = arg.toString(); if (typeof arg == "string") { arg = arg.replace(/(<(\w+)[^>]*?)\/>/g, function (m, all, tag) { return tag.match(/^(abbr|br|col|img|input|link|meta|param|hr|area)$/i) ? m : all + "></" + tag + ">"; }); var s = WidgetTelegramButton.trim(arg).toLowerCase(), div = doc.createElement("div"), tb = []; var wrap = !s.indexOf("<opt") && [1, "<select>", "</select>"] || !s.indexOf("<leg") && [1, "<fieldset>", "</fieldset>"] || s.match(/^<(thead|tbody|tfoot|colg|cap)/) && [1, "<table>", "</table>"] || !s.indexOf("<tr") && [2, "<table><tbody>", "</tbody></table>"] || (!s.indexOf("<td") || !s.indexOf("<th")) && [3, "<table><tbody><tr>", "</tr></tbody></table>"] || !s.indexOf("<col") && [2, "<table><tbody></tbody><colgroup>", "</colgroup></table>"] || WidgetTelegramButton.browser.msie && [1, "div<div>", "</div>"] || [0, "", ""]; div.innerHTML = wrap[1] + arg + wrap[2]; while (wrap[0]--) div = div.lastChild; if (WidgetTelegramButton.browser.msie) { if (!s.indexOf("<table") && s.indexOf("<tbody") < 0) tb = div.firstChild && div.firstChild.childNodes; else if (wrap[1] == "<table>" && s.indexOf("<tbody") < 0) tb = div.childNodes; for (var n = tb.length - 1; n >= 0; --n)if (WidgetTelegramButton.nodeName(tb[n], "tbody") && !tb[n].childNodes.length) tb[n].parentNode.removeChild(tb[n]); if (/^\s/.test(arg)) div.insertBefore(doc.createTextNode(arg.match(/^\s*/)[0]), div.firstChild); } arg = WidgetTelegramButton.makeArray(div.childNodes); } if (0 === arg.length && (!WidgetTelegramButton.nodeName(arg, "form") && !WidgetTelegramButton.nodeName(arg, "select"))) return; if (arg[0] == undefined || WidgetTelegramButton.nodeName(arg, "form") || arg.options) r.push(arg); else
                    r = WidgetTelegramButton.merge(r, arg);
            }); return r;
        ...
        add: function (element, type, handler, data) {
            ...
            var events = WidgetTelegramButton.data(element, "events"), ret, index; if (typeof type == "string") { var parts = type.split("."); type = parts[0]; } if (events) {
        ...
        show: function (speed, callback) { return speed ? this.animate({ height: "show", width: "show", opacity: "show" }, speed, callback) : this.filter(":hidden").each(function () { this.style.display = this.oldblock ? this.oldblock : ""; if (WidgetTelegramButton.css(this, "display") == "none") this.style.display = "block"; }).end(); }, hide: function (speed, callback) { return speed ? this.animate({ height: "hide", width: "hide", opacity: "hide" }, speed, callback) : this.filter(":visible").each(function () { this.oldblock = this.oldblock || WidgetTelegramButton.css(this, "display"); if (this.oldblock == "none") this.oldblock = "block"; this.style.display = "none"; }).end(); }, _toggle: WidgetTelegramButton.fn.toggle, toggle: function (fn, fn2) { return WidgetTelegramButton.isFunction(fn) && WidgetTelegramButton.isFunction(fn2) ? this._toggle(fn, fn2) : fn ? this.animate({ height: "toggle", width: "toggle", opacity: "toggle" }, fn, fn2) : this.each(function () { WidgetTelegramButton(this)[WidgetTelegramButton(this).is(":hidden") ? "show" : "hide"](); }); }, slideDown: function (speed, callback) { return this.animate({ height: "show" }, speed, callback); }, slideUp: function (speed, callback) { return this.animate({ height: "hide" }, speed, callback); }, slideToggle: function (speed, callback) { return this.animate({ height: "toggle" }, speed, callback); }, fadeIn: function (speed, callback) { return this.animate({ opacity: "show" }, speed, callback); }, fadeOut: function (speed, callback) { return this.animate({ opacity: "hide" }, speed, callback); }, fadeTo: function (speed, to, callback) { return this.animate({ opacity: to }, speed, callback); }, animate: function (prop, speed, easing, callback) {
            var opt = WidgetTelegramButton.speed(speed, easing, callback); return this[opt.queue === false ? "each" : "queue"](function () {
                opt = WidgetTelegramButton.extend({}, opt); var hidden = WidgetTelegramButton(this).is(":hidden"), self = this; for (var p in prop) { if (prop[p] == "hide" && hidden || prop[p] == "show" && !hidden) return WidgetTelegramButton.isFunction(opt.complete) && opt.complete.apply(this); if (p == "height" || p == "width") { opt.display = WidgetTelegramButton.css(this, "display"); opt.overflow = this.style.overflow; } } if (opt.overflow != null) this.style.overflow = "hidden"; opt.curAnim = WidgetTelegramButton.extend({}, prop); WidgetTelegramButton.each(prop, function (name, val) {
                    var e = new WidgetTelegramButton.fx(self, opt, name); if (/toggle|show|hide/.test(val)) e[val == "toggle" ? hidden ? "show" : "hide" : val](prop); else {
                        var parts = val.toString().match(/^([+-]?)([\d.]+)(.*)$/), start = e.cur(true) || 0; if (parts) { end = parseFloat(parts[2]), unit = parts[3] || "px"; if (unit != "px") { self.style[name] = end + unit; start = (end / e.cur(true)) * start; self.style[name] = start + unit; } if (parts[1]) end = ((parts[1] == "-" ? -1 : 1) * end) + start; e.custom(start, end, unit); } else
                            e.custom(start, val, "");
                    }
                }); return true;
            });
        }, queue: function (type, fn) { if (!fn) { fn = type; type = "fx"; } if (!arguments.length) return queue(this[0], type); return this.each(function () { if (fn.constructor == Array) queue(this, type, fn); else { queue(this, type).push(fn); if (queue(this, type).length == 1) fn.apply(this); } }); }, stop: function () { var timers = WidgetTelegramButton.timers; return this.each(function () { for (var i = 0; i < timers.length; i++)if (timers[i].elem == this) timers.splice(i--, 1); }).dequeue(); }
